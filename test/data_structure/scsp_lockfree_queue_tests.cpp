...
TEST(ScspLockFreeQueueTest, BasicOperations) {
  nyx::data_structure::ScspLockFreeQueue<int> queue(5);
...
TEST(ScspLockFreeQueueTest, MultiThread) {
  ...
  nyx::data_structure::ScspLockFreeQueue<int> queue(num_elements);

  auto producer = [&]() {
    for (int i = 0; i < num_elements; ++i) {
      while (!queue.push(i)) {
        std::this_thread::sleep_for(std::chrono::milliseconds(utils::time::now() % sleep_time));
      }
      std::this_thread::sleep_for(std::chrono::milliseconds(utils::time::now() % sleep_time));
    }
  };
